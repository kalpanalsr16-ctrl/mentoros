...
test("a misconception seen only once is excluded -- honest default threshold of 2", () => {
  const rows: ConfusionEventRow[] = [{ misconceptions: ["Forgets to carry the ten"], conceptName: "Addition", createdAt: "2026-01-01" }];
...
test("the exact same wording repeated crosses the threshold and is surfaced", () => {
  const rows: ConfusionEventRow[] = [
    { misconceptions: ["Forgets to carry the ten"], conceptName: "Addition with regrouping", createdAt: "2026-01-01" },
    { misconceptions: ["Forgets to carry the ten"], conceptName: "Addition with regrouping", createdAt: "2026-01-05" },
  ];
...
test("differently-worded descriptions of the same real confusion are NOT linked -- exact match only", () => {
  const rows: ConfusionEventRow[] = [
    { misconceptions: ["Forgets to carry the ten"], conceptName: "Addition", createdAt: "2026-01-01" },
    { misconceptions: ["Doesn't regroup when a column exceeds 9"], conceptName: "Addition", createdAt: "2026-01-05" },
  ];
  assert.deepEqual(aggregateStudentConfusion(rows), []);
});
...
test("results sort by frequency, most recurring first", () => {
  const rows: ConfusionEventRow[] = [
    { misconceptions: ["A"], conceptName: null, createdAt: "1" },
    { misconceptions: ["A"], conceptName: null, createdAt: "2" },
    { misconceptions: ["B"], conceptName: null, createdAt: "3" },
    { misconceptions: ["B"], conceptName: null, createdAt: "4" },
    { misconceptions: ["B"], conceptName: null, createdAt: "5" },
  ];
  const result = aggregateStudentConfusion(rows);
...
test("conceptNames dedupes across repeated occurrences in the same concept", () => {
  const rows: ConfusionEventRow[] = [
    { misconceptions: ["X"], conceptName: "Subtraction with regrouping", createdAt: "1" },
    { misconceptions: ["X"], conceptName: "Subtraction with regrouping", createdAt: "2" },
  ];
