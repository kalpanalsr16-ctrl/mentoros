...
All of Parts A, B, and C map to relational tables in the existing Supabase Postgres database — no new database technology. **Implemented in M5A** (`web/supabase/migrations/0002_curriculum_foundation.sql`): `subjects`, `grades`, `chapters`, `concepts`, `concept_relationships` (the graph edges, extensible `relationship_type`), `learning_objectives`, `concept_learning_objectives` (the many-to-many join), `misconceptions`, `teaching_strategies`, `mastery_criteria` — all with an authenticated-read-only RLS policy (curriculum content is shared reference data, not per-student data; authoring happens via migrations, not the app's client). Learning Resources (A6) remain undefined as their own table — no content type needing them has been authored yet; additive whenever it is.

Read access to this schema is intentionally split across two interfaces, not one: `KnowledgeProvider` (structured lookups by ID — `getConcept`, `getPrerequisites`, `getLearningObjectives`, `getMisconceptions`, `getTeachingStrategies`, `getMasteryCriteria`) and `ConceptSearchProvider` (resolving a free-text topic to a concept ID — `findConceptIdByTopic`). M5B's implementation (`search_concept_id()`, exact match then Postgres trigram similarity) is a real improvement over free-text guessing but still not semantic search — see M5's gate review for why an embedding provider was deliberately deferred rather than adopted here.
